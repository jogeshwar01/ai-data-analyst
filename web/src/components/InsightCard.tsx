import { TrendingDown, UserMinus, ArrowLeftRight, AlertCircle, BarChart2, PieChart, ChevronDown, ChevronUp } from "lucide-react";
  ...
  return (
    <div className="rounded-lg border border-zinc-800 bg-zinc-900 flex flex-col">
      <div className="px-4 pt-4 pb-3">
        ...
        <p className="text-xs text-zinc-400 mt-3 leading-relaxed">{card.narrative}</p>
      </div>
            ...
            <div className="overflow-x-auto border-t border-zinc-800">
              <table className="text-xs w-full border-collapse">
                <thead className="bg-zinc-800/50">
                  <tr>
                    {cols.map((c) => (
                      <th key={c} className="px-3 py-1.5 text-left font-medium text-zinc-300 border-b border-zinc-700">
                        {c}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {card.rows.slice(0, 8).map((row, i) => (
                    <tr key={i} className="even:bg-zinc-800/20">
                      {cols.map((c) => (
                        <td key={c} className="px-3 py-1.5 text-zinc-400 font-mono">
