...
def get_reps() -> list[dict]:
    _, rows = db.query("SELECT rep_id, first_name || ' ' || last_name AS name, region FROM rep_dim ORDER BY rep_id")
...
def get_coaching(rep_id: int) -> dict:
    ...
    under_covered = [
        r for r in hcp_rows
        if r[0] not in called_hcps or called_hcps[r[0]] < 3
    ]
    ...
    if under_covered:
        ...
        actions.append({
            ...
            "hcps": [{"hcp_id": r[0], "name": r[1], "tier": r[3], "total_trx": r[5], "your_calls": called_hcps.get(r[0], 0)} for r in under_covered],
        })
    ...
    if dormant_rows:
        ...
        actions.append({
            ...
            "hcps": [{"name": r[0], "specialty": r[1], "last_contact": str(r[2]) if r[2] else None, "days_since": r[3]} for r in dormant_rows],
        })
